...
function [savestate respath]=dssavefield(f,fnam,basedir,currpath,savestate,brakidx,task,checksaves,mapsave)
          ...
          for(k=rng)
            ...
            if(numel(varstosave)>0)
              ...
              if(mapsave&&dsbool(ds.sys.distproc,'mapreducer'))
                sendfile(filenm,ds.sys.distproc.reducehosts{k},ds.sys.distproc.myhost);
              end
...
function sendfile(fnam,targhost,myhost)
  global ds;
  ...
  fprintf(fid,['?=1\n']);
  fprintf(fid,['while [ "$?" -gt "0" ] ; do scp "' fnam '" "' targhost ':' fnam '_tmp"; done\n']);
  fprintf(fid,['?=1\n']);
  fprintf(fid,['while [ "$?" -gt "0" ] ; do ssh -F "' ds.sys.distproc.sshconf '" ' targhost ' mv "' fnam '_tmp" "' fnam '"; done\n']);
  fprintf(fid,['rm "' fnam '"\n']);
