...
ds.conf.params= struct( ...
  ...
  'samplingNumPerIm',20,...
  'subsamplenegs', negatives_per_positive/66, ...
  'includeflips', 1, ...
  'multperim', 1, ...
...
while(roundid<=(numel(ds.roundinds)))
  ...
  dsmapreduce(['detectors=effstrcell2mat(dsload(''ds.round.detectors'')'');'...
               ...
               'conf.thresh=-.02;'...
               '[dets,feats]=detectInIm(detectors,ds.round.myiminds(dsidx),conf);' ...
               ...
               'if(~isempty(dets)),'...
                 ...
                 '[dets,feats,outpos]=distributeby(dets,feats,ctridx(ctrpos,2));'...
                 'ds.round.newfeat(outpos,dsidx)=cellfun(@(x,y) struct(''assignedidx'',x,''feat'',y),dets,feats,''UniformOutput'',false);'...
               'end']...
  ...
  if(roundid>=4 && dsbool(ds.conf.params,'ovlweight'))
    [tokeep,scores,component]=findOverlapping3('ds.nextround.prevdets',unique(ds.batchfordetr(:,2)),[ds.batchfordetr(:,1),ds.classperbatch(ds.batchfordetr(:,2))],struct('ndetsforoverlap',.5,'maxoverlaps',3,'clusterer','agglomerative'));
  ...
  if(roundid==numel(ds.roundinds))
    ds.alldets=structcell2mat(dsload(['ds.nextround.prevdets'],'clear')');
    dssave;
    ds.alldets=[];
  end
...
for(i=numel(ds.roundinds)-2:numel(ds.roundinds))
  heldoutdets{end+1}=cell2mat(dsload(['ds.newdets{' num2str(i) '}{1:' num2str(dssavestatesize('ds.newdets',2)) '}'])');
  ds.newdets={};
end
...
dsrundistributed(['dsload(''ds.batchfordetr'');dsload(''ds.classperbatch'');dsload(''ds.imgs'');dsload(''ds.heldoutdets'');'...
  'legaldetrs=ds.batchfordetr(ds.classperbatch(ds.batchfordetr(:,2))==dsidx,1);'...
  'if(isempty(legaldetrs)),return;end,'...
  '[ds.finids{dsidx},ds.scores{dsidx}]=greedySelectDetrsCoverage(ds.heldoutdets,ds.imgs{ds.conf.currimset}.label==ds.uniquelabels(dsidx),.7,200,struct(''useoverlap'',1,''legaldetrs'',legaldetrs));'...
