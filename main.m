...
displayed_clusters = struct();
for (i = 1 : numel(best_clusters_idx))
  % the centroids
  centroid = initPatches(best_clusters_idx(i)); 
  displayed_clusters(i).centroid =  struct('img',centroid.im, 'patch',[centroid.x1 centroid.x2 centroid.y1 centroid.y2]);
  ...
  displayed_clusters(i).nn = nn;
  displayed_clusters(i).purity = purity(best_clusters_idx(i));
end

% generate a html page to see the results
generate_html_view(displayed_clusters, ds.imgs);