...
visualize = function(clusters){
  ...
  for (var i  = 0 ; i < 1000 ; i++){
    ...
    //cluster.id = i + 1;
    console.log(cluster);
    var d = _.template(template,{exp: experiment_name, img: cluster});
    $("#clusters").append(d);
  }
}
...
$(function() {
  ...
  if (experiment_name == '')
    experiment_name = 'period1_vs_period2';
  $("#experiment_id").html("experiment "+experiment_name);

  console.log(experiment_name);
  $.getJSON("../results/"+experiment_name+"/candidates/candidates.json", function(_clusters) {
    clusters = _clusters;
    visualize(clusters);   
  });
});